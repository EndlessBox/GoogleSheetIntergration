...
export async function handler(event: APIGatewayProxyEventV2, ctx: Context): Promise<APIGatewayProxyResultV2> {
	...
	if (path === "connect-url") {
		body = `${process.env.LightfunnelsFrontUrl}admin/oauth?client_id=${process.env.LightfunnelsAppKey}` +
			`&state=${state}&redirect_uri=${eventBody.redirectUrl}&response_type=code&scope=customers,orders&account-id=${eventBody.accountID}`;
		statusCode = 200;
			...
			rp({
				...
				.then(
					async function (data: { access_token: string, account_id: number, id: number }) {
						...
						const signedData: SignedData = { acid: id };
						let token = jsonwebtoken.sign(signedData, process.env.AppSecret);
						body = JSON.stringify({
							token,
							id: data.id
						});
