...
const fs = require("fs");
const { default: schema } = require("../bundle/backend/src/api/schema/index");
...
app.use(
  "/graphiql",
  convertExpressRequestToLambdaEventMiddleware(require("./gq/index").handler)
);
...
app.use(
  ...
  convertExpressRequestToLambdaEventMiddleware(
    require("../bundle/backend/src/webhooks/index").handler
  )
);
app.use(
  "/lightfunnels",
  convertExpressRequestToLambdaEventMiddleware(
    require("../bundle/backend/src/lightfunnels/index").handler
  )
);
app.use(
  "/api",
  convertExpressRequestToLambdaEventMiddleware(
    require("../bundle/backend/src/api/index.js").handler,
    {
      authorizer: require("../bundle/backend/src/authorizer/index.js").handler,
    }
  )
);
...
function convertExpressRequestToLambdaEventMiddleware(handler, opts = {}) {
  return async function routeHandler(req, res, next) {
    ...
    let event = {
      ...
      cookies: !req.cookies
        ? []
        : Object.keys(req.cookies).map((k) => `${k}=${req.cookies[k]}`),
    ...
    if (opts.authorizer) {
      let _res = await opts.authorizer(
        {
          identitySource: [req.headers["authorization"] || ""],
        },
        {}
      );
